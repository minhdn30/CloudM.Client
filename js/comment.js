...
const CommentModule = (function () {
  ...
  async function handleLikeComment(commentId, container, event) {
    ...
    if (event) {
        ...
        if (clickedCount && window.InteractionModule) {
            ...
            InteractionModule.openReactList(commentId, 'comment', countEl.textContent);
            return;
        }
    }
  ...
  async function loadReplies(commentId, page = 1) {
    ...
    if (page === 1) repliesList.innerHTML = `<div class="feed-loader replies-loading"><div class="spinner spinner-small"></div></div>`;
    ...
    try {
        ...
        if (data.hasNextPage) {
            ...
            const countText = (totalCount && totalCount > 0) ? ` (${totalCount})` : "";
            ...
            const firstSessionReplyAfterBatch = repliesList.querySelector(".session-reply");
  ...
  function injectUpdatedComment(comment, forceExpand = false) {
    ...
    if (textEl && window.PostUtils) {
      PostUtils.setupCommentContent(textEl, comment.content, undefined, forceExpand);
      
      const timeEl = el.querySelector(".comment-time");
      if (timeEl) {
        timeEl.title = PostUtils.formatFullDateTime(comment.updatedAt || comment.createdAt);
      }
      ...
      if (footer) {
        ...
        if (comment.updatedAt && comment.updatedAt !== comment.createdAt) {
          if (!editedEl) {
            ...
            editedEl.title = "Edited at " + PostUtils.formatFullDateTime(comment.updatedAt);
          }
        }
      }
    }
  ...
  function showDeleteConfirm(commentId, isReply) {
      ...
      document.getElementById("confirmDeleteBtn").onclick = async () => {
          ...
          try {
              ...
              const selector = isReply ? `[data-reply-id="${commentId}"]` : `[data-comment-id="${commentId}"]`;
              const el = document.querySelector(selector);
              if (el) {
                  el.style.opacity = '0';
  ...
  function editComment(commentId, isReply) {
    ...
    const emojiPickerContainer = body.querySelector(".edit-emoji-picker-container");
    ...
    input.oninput = () => {
      ...
      saveBtn.disabled = newContent === originalContent || newContent.length === 0;
      autoResize();
    };
    ...
    cancelBtn.onclick = () => {
      ...
      if (newTextEl) PostUtils.setupCommentContent(newTextEl, originalContent, undefined, true);
    };
  ...
  function getEditedHtml(comment) {
    if (!comment.updatedAt || comment.updatedAt === comment.createdAt) return "";
