...
const ChatActions = {
  ...
  uiError(action, status = 0, rawMessage = "", fallbackKey = "errors.generic", fallback = "Something went wrong.") {
    ...
    const uiError = window.UIErrors?.format?.("chat", action, status, rawMessage);
  ...
  bindLanguageChange() {
    ...
    window.I18n.onChange(() => {
      const reactorsMessageId = (this.currentReactorsModal?.dataset?.messageId || "")
        .toString()
        .toLowerCase();
  ...
  getThemeTargetFromElement(element) {
    if (element?.closest) {
      const scopedTarget = element.closest(
        ".chat-box, .chat-view, .chat-panel-container",
      );
  ...
  getThemeTargetByConversationId(conversationId) {
    ...
    if (normalizedConversationId) {
      const chatBox =
        document.getElementById(`chat-box-${normalizedConversationId}`) ||
        document.querySelector(
          `.chat-box[data-id="${normalizedConversationId}"]`,
        );
      if (chatBox) return this.getThemeTargetFromElement(chatBox);

      const messageList = document.getElementById(
        `chat-messages-${normalizedConversationId}`,
      );
      if (messageList) return this.getThemeTargetFromElement(messageList);

      const pageConversationId = (window.ChatPage?.currentChatId || "")
        .toString()
        .toLowerCase();
      if (
        pageConversationId &&
        pageConversationId === normalizedConversationId
      ) {
  ...
  applyFloatingThemeVars(targetElement, themeTarget, vars = null) {
    ...
    const resolvedThemeTarget =
      themeTarget ||
      targetElement.__chatThemeSource ||
      this.getThemeTargetByConversationId(
        targetElement.dataset?.conversationId,
      ) ||
    ...
    const themeClass = [...resolvedThemeTarget.classList].find((className) =>
  ...
  syncReactionThemeStyles() {
    ...
    if (
      currentMenu?.classList?.contains("msg-react-menu") ||
      currentMenu?.classList?.contains("chat-pinned-more-menu")
    ) {
      ...
      if (messageId) themeTarget = this.getThemeTargetByMessageId(messageId);
        ...
        themeTarget = this.getThemeTargetByConversationId(conversationId);

      this.applyFloatingThemeVars(
        currentMenu,
        themeTarget,
        this.getReactionThemeVars(),
      );
    ...
    if (reactorsOverlay?.classList?.contains("msg-reactors-overlay")) {
      const themeTarget = this.getThemeTargetByMessageId(
        reactorsOverlay.dataset?.messageId,
      );
      this.applyFloatingThemeVars(
        reactorsOverlay,
        themeTarget,
    ...
    if (pinnedOverlay?.classList?.contains("chat-pinned-overlay")) {
      const themeTarget = this.getThemeTargetByConversationId(
        pinnedOverlay.dataset?.conversationId,
      );
      this.applyFloatingThemeVars(
        pinnedOverlay,
        themeTarget,
  ...
  normalizeReactState(payload, fallbackMessageId = "") {
    ...
    const reacts = window.ChatCommon?.normalizeMessageReactSummaries
      ? window.ChatCommon.normalizeMessageReactSummaries(
          payload?.reacts || payload?.Reacts,
        )
      : [];
    const reactedBy = window.ChatCommon?.normalizeMessageReactedBy
      ? window.ChatCommon.normalizeMessageReactedBy(
          payload?.reactedBy || payload?.ReactedBy,
        )
      : [];
    ...
    if (
      ...
      Array.isArray(reactionData.reactedBy)
    ) {
  ...
  updateMessageReactStateUI(messageId, state) {
    ...
    bubbles.forEach((bubble) => {
      ...
      const reactBtn = bubble.querySelector(
        '.msg-action-btn.react[data-action="react"]',
      );
      if (reactBtn) {
  ...
  markReactMenuActiveOption(menuEl, reactType) {
    ...
    menuEl.querySelectorAll(".msg-react-option").forEach((btn) => {
      ...
      btn.classList.toggle(
        "active",
        normalizedType !== null && optionType === normalizedType,
      );
    });
  },
  ...
  async setMessageReact(messageId, reactType) {
    ...
    if (
      !normalizedMessageId ||
      normalizedType === null ||
      !window.API?.Messages?.setReact
    )
      return null;

    const res = await window.API.Messages.setReact(
      normalizedMessageId,
      normalizedType,
    );
    if (!res?.ok) {
  ...
  openReactMenu(e, messageId) {
    ...
    const resolvedMessageId = (messageId || wrapper?.dataset?.messageId || "")
      .toString()
      .toLowerCase();
    ...
    menu.innerHTML = this._reactOptions
      .map(
        (option) => `
    ...
    menu.querySelectorAll(".msg-react-option").forEach((optionBtn) => {
      optionBtn.onclick = async (evt) => {
        ...
        try {
          const nextState = await this.setMessageReact(
            resolvedMessageId,
            selectedType,
          );
          this.markReactMenuActiveOption(
            menu,
            nextState?.currentUserReactType ?? null,
          );
        } catch (err) {
    ...
    this.getMessageReactState(resolvedMessageId)
      .then((state) => {
        ...
        this.markReactMenuActiveOption(
          menu,
          state?.currentUserReactType ?? null,
        );
      })
      .catch(() => {
  ...
  getReactOption(reactType) {
    ...
    return (
      this._reactOptions.find((option) => option.type === normalizedType) ||
      null
    );
  },
  ...
  closeReactorsModal() {
    ...
    setTimeout(() => {
      ...
      if (this.currentReactorsModal === overlay)
        this.currentReactorsModal = null;
    }, 180);
  },
  ...
  async openReactorsModal(messageId, event = null, prefetchedState = null) {
    ...
    if (!state) {
      try {
        ...
        if (window.toastError) {
          window.toastError(
            this.t("chat.actions.react.loadFailed", {}, "Failed to load reactions."),
          );
        }
        return;
      }
    ...
    popup.innerHTML = `
            ...
            <div class="msg-reactors-list">
                ${reactedBy
                  .map((item) => {
                    const displayName = window.ChatCommon
                      ?.getMessageReactionDisplayName
  ...
  replyTo(messageId) {
    ...
    if (senderId) {
      ...
      if (meta) {
        if (!meta.isGroup && meta.otherMember) {
          ...
          if (senderId === otherId) {
            senderName =
              meta.otherMember.nickname ||
              meta.otherMember.username ||
              meta.otherMember.fullName ||
              "";
          }
        }
        if (!senderName && Array.isArray(meta.members)) {
          const member = meta.members.find(
            (m) => (m.accountId || "").toLowerCase() === senderId,
          );
          if (member) {
            senderName =
      ...
      if (!senderName && window.ChatWindow) {
        for (const [, chat] of window.ChatWindow.openChats || []) {
          ...
          if (
            chatMeta.otherMember &&
            (chatMeta.otherMember.accountId || "").toLowerCase() === senderId
          ) {
            senderName =
              chatMeta.otherMember.nickname ||
              chatMeta.otherMember.username ||
              chatMeta.otherMember.fullName ||
              "";
            break;
          }
        }
    ...
    if (isRecalled) {
      ...
      if (bubbleText) {
        contentPreview =
          bubbleText.length > 60
            ? bubbleText.substring(0, 60) + "\u2026"
            : bubbleText;
      } else if (
  ...
  getHiddenMessageText() {
    ...
    if (window.I18n?.translateServerText && window.APP_CONFIG?.CHAT_HIDDEN_MESSAGE_TEXT) {
  ...
  updateReplyBarsForParentState(messageId, state = {}) {
    ...
    const previewText = isHidden
      ...
      : isRecalled
        ? this.getRecalledMessageText()
        : "";
    if (!previewText) return;

    if (
      window.ChatPage &&
      (window.ChatPage._replyToMessageId || "").toString().toLowerCase() ===
        normalizedMessageId
    ) {
      window.ChatPage._replyContentPreview = previewText;
      const pagePreviewEl = document.querySelector(
        ".chat-view-input-container .chat-reply-bar .chat-reply-bar-preview",
      );
    ...
    if (window.ChatWindow?.openChats) {
      for (const [chatId, chat] of window.ChatWindow.openChats.entries()) {
        ...
        const previewEl = inputArea?.querySelector(
          ".chat-reply-bar .chat-reply-bar-preview",
        );
  ...
  updateReplyPreviewStateForParent(messageId, state = {}) {
    ...
    const previews = document.querySelectorAll(
      `.msg-reply-preview[data-reply-id="${normalizedMessageId}"]`,
    );
    if (!previews.length) {
    ...
    previews.forEach((previewEl) => {
      if (!previewEl) return;
      const wasHidden =
        (previewEl.dataset.replyParentHidden || "").toLowerCase() === "true";
      if (!isHidden && wasHidden) return; // Hidden state has priority for current user.

      previewEl.dataset.replyParentHidden = isHidden
        ? "true"
        : wasHidden
          ? "true"
          : "false";
  ...
  resolveConversationId(wrapper) {
    const fromWrapper = (wrapper?.dataset?.conversationId || "")
      .toString()
      .toLowerCase();
    ...
    const pageId = (window.ChatPage?.currentChatId || "")
      .toString()
      .toLowerCase();
  ...
  extractSidebarMessageFromBubble(bubble) {
    if (!bubble || !bubble.classList?.contains("msg-bubble-wrapper"))
      return null;
    ...
    if (isSystemMessage) {
      const systemText = (
        bubble.querySelector(".msg-system-text")?.textContent || ""
      ).trim();
      return {
        messageId,
        sentAt,
  ...
  getConversationMeta(conversationId) {
    ...
    if (
      window.ChatPage &&
      (window.ChatPage.currentChatId || "").toString().toLowerCase() ===
    ...
    if (Array.isArray(window.ChatSidebar?.conversations)) {
      const found = window.ChatSidebar.conversations.find(
        (c) =>
          (c?.conversationId || c?.ConversationId || "")
            .toString()
            .toLowerCase() === normalizedConversationId,
      );
      return found || null;
    }
  ...
  canPinConversation(conversationId) {
    ...
    const ownerId = (meta?.owner ?? meta?.Owner ?? "")
      .toString()
      .toLowerCase()
      .trim();
  ...
  removePinnedModalItem(messageId) {
    ...
    if (!list.querySelector(".chat-pinned-item")) {
      list.innerHTML =
        `<div class="chat-pinned-empty">${this.escapeHtml(
          this.t(
  ...
  async pinMessage(conversationId, messageId) {
    ...
    this.closeAllMenus();
    try {
      const res = await window.API.Messages.pin(
        normalizedConversationId,
        normalizedMessageId,
      );
      if (!res.ok) {
  ...
  async unpinMessage(conversationId, messageId) {
    ...
    this.closeAllMenus();
    try {
      const res = await window.API.Messages.unpin(
        normalizedConversationId,
        normalizedMessageId,
      );
      if (!res.ok) {
  ...
  togglePinnedTruncate(btn) {
    const msgEl = btn
      ?.closest(".chat-pinned-item-content")
  ...
  renderPinnedLoadMoreState(listEl, state, options = {}) {
    ...
    if (state.isLoading && options.append) {
      footer.innerHTML =
        `<div class="chat-pinned-loading"><span class="spinner spinner-small" aria-hidden="true"></span><span>${this.escapeHtml(
    ...
    if (!state.hasNextPage) {
      footer.innerHTML =
        `<div class="chat-pinned-loading done">${this.escapeHtml(
          this.t(
  ...
  async showPinnedMessages(conversationId, options = {}) {
    ...
    const title =
      options.title ||
      this.getPinnedConversationTitle(normalizedConversationId);
    const overlay = document.createElement("div");
    ...
    if (window.lockScroll) lockScroll();
    if (window.lucide)
      lucide.createIcons({ props: { size: 18 }, container: overlay });
    requestAnimationFrame(() => overlay.classList.add("show"));
  ...
  refreshPinnedModalIfOpen(conversationId) {
    ...
    const currentConversationId = (overlay.dataset?.conversationId || "")
      .toString()
      .toLowerCase();
  ...
  findMessageElementForConversation(conversationId, normalizedMessageId) {
    ...
    if (
      ...
      const pageTarget = this.findMessageElementInContainer(
        pageContainer,
        normalizedMessageId,
      );
    ...
    if (window.ChatWindow && normalizedConvId) {
      ...
      if (openId) {
        const windowContainer = document.getElementById(
          `chat-messages-${openId}`,
        );
        const windowTarget = this.findMessageElementInContainer(
          windowContainer,
          normalizedMessageId,
        );
    ...
    const globalExact = document.querySelector(
      `.msg-bubble-wrapper[data-message-id="${normalizedMessageId}"]`,
    );
          ...
          window.toastInfo &&
            window.toastInfo(
              this.t(
                ...
                {},
                "Could not locate message in current chat.",
              ),
            );
        });
    };
  ...
  openMoreMenu(e, messageId, isOwn) {
    ...
    // Inherit theme
    const themeTarget =
      btn.closest(".chat-box") ||
      btn.closest(".chat-view") ||
      btn.closest(".chat-panel-container");
    if (themeTarget) {
      const vars = [
        "--chat-theme-surface",
        "--chat-theme-border",
      ...
      const themeClass = [...themeTarget.classList].find((c) =>
        c.startsWith("chat-theme-"),
      );
    ...
    if (canForward) {
      itemsHtml += `
            <div class="msg-more-item" onclick="window.ChatActions && ChatActions.closeAllMenus(); if(window.openForwardMessageModal){ window.openForwardMessageModal('${resolvedMessageId}'); } else if(window.toastInfo){ window.toastInfo('${this.t("chat.actions.menu.forwardUnavailable", {}, "Forward is unavailable.").replace(/'/g, "\\'")}'); }">
    ...
    if (!isOwn) {
      itemsHtml += `
                <div class="msg-more-item danger" onclick="window.toastInfo('${this.t("chat.actions.menu.reportSuccess", {}, "Reported successfully.").replace(/'/g, "\\'")}')">
  ...
  closeAllMenus() {
    ...
    if (
      window.ChatWindow &&
      typeof window.ChatWindow.closeHeaderMenu === "function"
    ) {
    ...
    document
      ...
      .forEach((w) => w.classList.remove("menu-active"));
    document
  ...
  handleOutsideClick(e) {
    if (
      ChatActions.currentMenu &&
      !ChatActions.currentMenu.contains(e.target)
    ) {
      ChatActions.closeAllMenus();
    }
  },
  ...
  hideForYou(messageId) {
    ...
    if (!normId) {
      window.toastError &&
        window.toastError(
          this.t("chat.actions.hide.failed", {}, "Failed to hide message."),
        );
      return;
    }
    this.closeAllMenus();
  ...
  removeMessageFromUI(messageId, fallbackConversationId = "") {
    if (!messageId) return false;
    const normId = messageId.toString().toLowerCase();
    const fallbackConvId = (fallbackConversationId || "")
      .toString()
      .toLowerCase();
    this.markMessageHiddenForCurrentUser(normId, true);
    const updatedReplyPreviews = this.updateReplyPreviewStateForParent(normId, {
      hidden: true,
    });
    const modalMessageId = (this.currentReactorsModal?.dataset?.messageId || "")
      .toString()
      .toLowerCase();
    ...
    bubbles.forEach((bubble) => {
      ...
      const container =
        ...
        bubble.closest(".chat-view-messages");
      const conversationId =
        this.resolveConversationId(bubble) || fallbackConvId;
      ...
      setTimeout(() => {
        ...
        if (
          ...
          try {
            window.ChatSidebar.applyMessageHidden(
              conversationId,
              normId,
              replacementLastMessage,
            );
          } catch (err) {
  ...
  reassignSentStatus(container) {
    ...
    const isLastRecalled =
      (lastBubble.dataset?.isRecalled || "").toString().toLowerCase() ===
      "true";
    if (
      isLastRecalled ||
      window.ChatCommon?.isSystemMessageElement?.(lastBubble)
    )
      return;
  ...
  buildMessageShapeFromElement(el) {
    ...
    const typeRaw = (el.dataset?.messageType || "")
      .toString()
      .trim()
      .toLowerCase();
    let messageType = null;
    if (
      el.classList.contains("msg-system") ||
      typeRaw === "system" ||
      typeRaw === "3"
    ) {
      messageType = 3;
    ...
    return {
      sender: {
        accountId: (el.dataset?.senderId || "").toString().toLowerCase(),
      },
  ...
  refreshMessageState(el) {
    ...
    if (
      m.messageType === 3 ||
      window.ChatCommon?.isSystemMessageElement?.(el)
    ) {
      const classes = [
        "msg-group-first",
        "msg-group-middle",
    ...
    if (avatarContainer) {
      const shouldShowAvatar =
        !isOwn && (newGroupPos === "last" || newGroupPos === "single");
      if (shouldShowAvatar) {
        avatarContainer.classList.remove("msg-avatar-spacer");
        if (!avatarContainer.querySelector("img")) {
          const avatarUrl =
            el.dataset.avatarUrl || window.APP_CONFIG?.DEFAULT_AVATAR;
    ...
    if (authorName && !isOwn) {
      const shouldShowName =
        newGroupPos === "first" || newGroupPos === "single";
  ...
  cleanTimeSeparators(container) {
    ...
    separators.forEach((sep) => {
      ...
      const shouldKeep =
        Number.isFinite(prevTime.getTime()) &&
  ...
  handleMediaRecallEffects(messageId, conversationId = "") {
    ...
    if (
      window.ChatPage &&
      typeof window.ChatPage.removeRecalledMediaFromPanel === "function"
    ) {
      try {
        removedFromPanel =
          Number(
            window.ChatPage.removeRecalledMediaFromPanel(
              normalizedMessageId,
              normalizedConversationId,
            ),
          ) || 0;
    ...
    if (
      window.ChatPage &&
      typeof window.ChatPage.removeRecalledFilesFromPanel === "function"
    ) {
      try {
        window.ChatPage.removeRecalledFilesFromPanel(
          normalizedMessageId,
          normalizedConversationId,
        );
      } catch (err) {
    ...
    if (
      !previewer ||
      typeof previewer.isOpen !== "function" ||
      !previewer.isOpen()
    )
      return;

    const previewSource = (
      typeof previewer.getSource === "function" ? previewer.getSource() : ""
    ).toLowerCase();
    const previewConversationId = (
      typeof previewer.getConversationId === "function"
    ...
    if (previewSource === "conversation-media-panel") {
      if (
        ...
        normalizedConversationId !== previewConversationId
      ) {
        return;
      }
  ...
  applyRecalledState(messageId, options = {}) {
    ...
    bubbles.forEach((bubble) => {
      ...
      bubble
        .querySelectorAll(
          ".msg-media-anchor, .msg-file-anchor, .msg-media-grid, .msg-file-list, .msg-file-item, .msg-reactions-summary, .msg-post-share-preview, .msg-story-reply-preview",
        )
        .forEach((el) => el.remove());

      const contentContainers = bubble.querySelectorAll(
  ...
  recallFromRealtime(data) {
    const messageId = data?.messageId || data?.MessageId;
    let conversationId = (data?.conversationId || data?.ConversationId || "")
      .toString()
      .toLowerCase();
    if (!messageId) return;
    if (!conversationId) {
    ...
    if (
      conversationId &&
      window.ChatSidebar &&
      typeof window.ChatSidebar.applyMessageRecalled === "function"
    ) {
  ...
  recallMessage(messageId) {
    ...
    if (!normId) {
      window.toastError &&
        window.toastError(
          this.t("chat.actions.recall.failed", {}, "Failed to recall message."),
        );
      return;
    }
  ...
  showConfirm(title, message, onConfirm, options = {}) {
    ...
    if (activeMenu) {
      const themeClass = [...activeMenu.classList].find((c) =>
        c.startsWith("chat-theme-"),
      );
