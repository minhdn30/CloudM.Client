...
const cgGetSearchLimit = () =>
  window.APP_CONFIG?.GROUP_CHAT_INVITE_SEARCH_LIMIT || 10;
const cgGetSearchDebounceMs = () =>
  window.APP_CONFIG?.GROUP_CHAT_INVITE_SEARCH_DEBOUNCE_MS || 300;
const cgGetAvatarMaxSizeMb = () =>
  window.APP_CONFIG?.GROUP_CHAT_AVATAR_MAX_SIZE_MB || 5;
const cgGetAvatarMaxBytes = () => cgGetAvatarMaxSizeMb() * 1024 * 1024;
const cgGetDefaultAvatar = () =>
  window.APP_CONFIG?.DEFAULT_AVATAR || "assets/images/default-avatar.jpg";
const cgGetGroupNameMinLength = () =>
  window.APP_CONFIG?.GROUP_NAME_MIN_LENGTH || 3;
...
window.cgClearGroupAvatarPreview = () =>
  cgResetAvatarPreview({ clearInput: true });
...
function cgDoesMemberMatchKeyword(member, keyword) {
  ...
  return (
    fullName.includes(normalizedKeyword) || username.includes(normalizedKeyword)
  );
}
...
function cgToggleMember(id, name, avatar, username) {
  ...
  if (item) {
    item.classList.toggle(
      "selected",
      cgSelectedMembers.some((m) => m.id === id),
    );
  }
...
async function cgHandleCreateGroup() {
  ...
  const memberIds = Array.from(
    new Set(cgSelectedMembers.map((m) => m.id).filter(Boolean)),
  );
  const avatarFile =
    document.getElementById("group-avatar-input")?.files?.[0] || null;
  ...
  try {
    ...
    const conversationId =
      conversation?.conversationId || conversation?.ConversationId || null;
    ...
    const currentPath =
      window.RouteHelper && typeof window.RouteHelper.parseHash === "function"
        ...
        : "";
    const isOnChatPage =
      window.RouteHelper && typeof window.RouteHelper.isChatPath === "function"
        ? window.RouteHelper.isChatPath(currentPath)
        : window.location.hash.startsWith("#/chat") ||
          window.location.hash.startsWith("#/messages");
    const shouldOpenConversation = !!conversationId && isOnChatPage;
    if (shouldOpenConversation) {
      if (
        ...
        window.location.hash =
          window.RouteHelper &&
          typeof window.RouteHelper.buildHash === "function"
